...
> [!IMPORTANT]
> Version `0.1.1` is a development release. Its automated test suite passes, but the module has not yet completed the live Foundry test matrix. See [Testing status](#testing-status) before using it in an active game.
...
Live Foundry validation is not yet complete, so version `0.1.1` should be treated as a development release. Please report unexpected behavior through GitHub Issues.
